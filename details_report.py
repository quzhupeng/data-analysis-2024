# -*- coding: utf-8 -*-
"""
鐢熸垚璇︾粏鏁版嵁鍒嗘瀽鎶ュ憡椤甸潰 (details.html)銆?鍖呭惈姣忔棩浜ч攢鐜囨槑缁嗗拰姣忔棩閿€鍞槑缁嗐€?"""
...
def _generate_product_sales_ratio_detail_panel(date_str, data):
    ...
    if data is None or data.empty:
        ...
        return f'''<div id="ratioPanel_{date_str}" class="ratio-panel" style="display: none;">
                 <div class="ratio-panel-header"><h4>浜ч攢鐜囨槑缁?- {date_str}</h4><button onclick="togglePanel('ratioPanel_{date_str}', event)" class="close-button"><i class="bi bi-x-lg"></i></button></div>
    ...
    <div id="ratioPanel_{date_str}" class="ratio-panel" style="display: none;">
        <div class="ratio-panel-header">
            ...
            <div class="ratio-panel-controls">
                <input type="text" id="ratioSearch_{date_str}" onkeyup="searchTableInPanel('ratioTable_{date_str}', 'ratioSearch_{date_str}')" placeholder="鎼滅储浜у搧..." class="search-input">
        ...
        <div class="ratio-panel-body">
            <div class="sales-table-container table-responsive">
                <table id="ratioTable_{date_str}" class="mobile-friendly-table">
                    <thead><tr><th>浜у搧鍚嶇О</th><th>閿€閲?/th><th>浜ч噺</th><th>浜ч攢鐜?/th></tr></thead>
                    <tbody>
    '''
    try:
    ...
    if not all(col in data_sorted.columns for col in required_cols):
        ...
        for _, row in data_sorted.iterrows():
            ...
            sales_val = row.get('閿€閲?); prod_val = row.get('浜ч噺'); ratio_val = row.get('浜ч攢鐜?)
        ...
        try:
            ...
            panel_html += f'''<tr class="total-row"><td><strong>鍚堣</strong></td><td class="text-right"><strong>{total_sales:,.0f}</strong></td><td class="text-right"><strong>{total_production:,.0f}</strong></td><td class="{total_ratio_class} text-right"><strong>{total_ratio:.0f}%</strong></td></tr>'''
        except Exception: panel_html += "<tr><td colspan='4'>璁＄畻鍚堣鏃跺嚭閿欍€?/td></tr>"
...
def _generate_ratio_detail_section(product_sales_ratio_data):
    """鐢熸垚姣忔棩浜у搧浜ч攢鐜囨槑缁嗛儴鍒?(鍗＄墖 + 闈㈡澘)"""
    if not product_sales_ratio_data or len(product_sales_ratio_data) == 0:
        return '''<div id="daily-ratio-details" class="section"><div class="section-header"><h2>姣忔棩浜у搧浜ч攢鐜囨槑缁?/h2><a href="ratio.html" class="btn btn-secondary btn-sm back-button">杩斿洖浜ч攢鐜囨€昏</a></div><div class="section-body"><p>鏃犱骇鍝佷骇閿€鐜囨槑缁嗘暟鎹€?/p></div></div>'''

    detail_html = '''
    <div id="daily-ratio-details" class="section">
        <div class="section-header">
            <h2>姣忔棩浜у搧浜ч攢鐜囨槑缁?/h2>
            <a href="ratio.html" class="btn btn-secondary btn-sm back-button">杩斿洖浜ч攢鐜囨€昏</a>
        </div>
        <div class="section-body">
            <p>鐐瑰嚮涓嬫柟鍗＄墖鏌ョ湅褰撴棩鍚勪骇鍝佺殑浜ч攢鐜囪鎯?(浜ч攢鐜?= 閿€閲?/ 浜ч噺 脳 100%)銆?/p>
            <p style="text-align: right; font-size: 12px; color: #666; margin-top: 5px;">鏁版嵁鏉ユ簮锛氫骇鎴愬搧鍏ュ簱鍒楄〃 & 閿€鍞彂绁ㄦ墽琛屾煡璇紙鎺掗櫎瀹㈡埛鍚嶇О涓虹┖銆佸壇浜у搧銆侀矞鍝佺殑璁板綍锛?/p>
    ...
    for day_data_item in sorted_data_items:
        ...
        if data_df is not None and not data_df.empty and '閿€閲? in data_df.columns and '浜ч噺' in data_df.columns:
            try:
                total_sales = data_df['閿€閲?].sum(); total_production = data_df['浜ч噺'].sum()
        ...
        detail_html += f'''
            <div class="sales-flex-item">
                <div class="sales-card" onclick="toggleRatioPanel('{date_str}', event)">
                    ...
                    <div class="sales-card-body"><p><i class="bi bi-speedometer2"></i> 骞冲潎: <strong>{avg_ratio:.1f}%</strong></p><p><i class="bi bi-collection"></i> 浜у搧鏁? <strong>{product_count}</strong></p></div>
                    <div class="card-hint">鐐瑰嚮鏌ョ湅璇︽儏</div>
                </div>
                </div>
        '''
...
def _generate_sales_detail_panel(date_str, sales_data, quantity_column):
    """鐢熸垚姣忔棩閿€鍞槑缁嗛潰鏉跨殑HTML銆?""
    if sales_data is None or sales_data.empty:
         # Return an empty div with the panel ID so JS doesn't break
        return f'''<div id="salesPanel_{date_str}" class="sales-panel" style="display: none;">
                 <div class="sales-panel-header"><h4>閿€鍞槑缁?- {date_str}</h4><button onclick="togglePanel('salesPanel_{date_str}', event)" class="close-button"><i class="bi bi-x-lg"></i></button></div>
                 <div class="sales-panel-body"><p>褰撴棩鏃犺缁嗛攢鍞暟鎹€?/p></div>
               </div>'''

    panel_html = f'''
    <div id="salesPanel_{date_str}" class="sales-panel" style="display: none;">
        <div class="sales-panel-header">
            <h4>閿€鍞槑缁?- {date_str}</h4>
            <div class="sales-panel-controls">
                <input type="text" id="salesSearch_{date_str}" onkeyup="searchTableInPanel('salesTable_{date_str}', 'salesSearch_{date_str}')" placeholder="鎼滅储浜у搧..." class="search-input">
        ...
        <div class="sales-panel-body">
            <div class="sales-table-container table-responsive">
                <table id="salesTable_{date_str}" class="mobile-friendly-table">
                    <thead><tr><th>鐗╂枡鍚嶇О</th><th>閿€閲?/th><th>閿€鍞噾棰?鏃犵◣)</th><th>鍚◣鍗曚环(鍏?鍚?</th></tr></thead>
                    <tbody>
    '''
    ...
    try:
        ...
        elif '鏈竵鏃犵◣閲戦' in sales_data.columns and pd.api.types.is_numeric_dtype(sales_data['鏈竵鏃犵◣閲戦']): sorted_sales = sales_data.sort_values(by='鏈竵鏃犵◣閲戦', ascending=False, na_position='last')
    except Exception: pass

    required_cols = ['鐗╂枡鍚嶇О', '鏈竵鏃犵◣閲戦', '鍚◣鍗曚环']
    if quantity_column: required_cols.append(quantity_column)
    if not all(col in sorted_sales.columns for col in required_cols):
         panel_html += "<tr><td colspan='4'>閿€鍞槑缁嗘暟鎹垪涓嶅畬鏁淬€?/td></tr>"
    else:
        for _, row in sorted_sales.iterrows():
            mat_name = row.get('鐗╂枡鍚嶇О', 'N/A'); amount_val = row.get('鏈竵鏃犵◣閲戦'); unit_price_val = row.get('鍚◣鍗曚环'); quantity_val = row.get(quantity_column) if quantity_column else None
        ...
        try:
             ...
             total_amount = sorted_sales['鏈竵鏃犵◣閲戦'].sum() if '鏈竵鏃犵◣閲戦' in sorted_sales.columns and pd.api.types.is_numeric_dtype(sorted_sales['鏈竵鏃犵◣閲戦']) else 0
             ...
             if total_volume is not None and total_volume > 0 and total_amount is not None:
                 try:
                     ...
                     # Assuming total_amount is 鏃犵◣閲戦 and total_volume is in KG
                     avg_price = (total_amount / total_volume) * 1.09 * 1000
                     total_avg_price_display = f"{avg_price:,.0f}"
                 except ZeroDivisionError:
                     total_avg_price_display = "鏃犳硶璁＄畻"
                 except Exception:
                     total_avg_price_display = "璁＄畻閿欒"
...
def _generate_sales_detail_section(daily_sales):
    """鐢熸垚姣忔棩閿€鍞槑缁嗛儴鍒?(鍗＄墖 + 闈㈡澘)"""
    if not daily_sales or len(daily_sales) == 0:
        return '''<div id="daily-sales-details" class="section"><div class="section-header"><h2>姣忔棩閿€鍞槑缁?/h2><a href="sales.html" class="btn btn-secondary btn-sm back-button">杩斿洖閿€鍞€昏</a></div><div class="section-body"><p>鏃犻攢鍞槑缁嗘暟鎹彲渚涙樉绀恒€?/p></div></div>'''

    detail_html = '''
    <div id="daily-sales-details" class="section">
        <div class="section-header">
            <h2>姣忔棩閿€鍞槑缁?/h2>
            <a href="sales.html" class="btn btn-secondary btn-sm back-button">杩斿洖閿€鍞€昏</a>
        </div>
        <div class="section-body">
            <p>鐐瑰嚮涓嬫柟鍗＄墖鏌ョ湅褰撴棩鍚勪骇鍝佺殑閿€鍞鎯呫€?/p>
    ...
    for date in sorted_dates:
        ...
        volume_display = "-"; avg_price_display = "鏃犳硶璁＄畻"
        ...
        if daily_avg_price is not None:
            try: avg_price_display = f"{float(daily_avg_price):,.0f} 鍏?鍚?
            except: avg_price_display = f"{str(daily_avg_price)} 鍏?鍚?
            ...
            <div class="sales-flex-item">
                <div class="sales-card" onclick="toggleSalesPanel('{date_str}', event)">
                    ...
                    <div class="sales-card-body">
                        <p><i class="bi bi-box"></i> 閿€閲? <strong>{volume_display}</strong></p>
                        <p><i class="bi bi-currency-yen"></i> 鍧囦环: <strong>{avg_price_display}</strong></p>
                        <p><i class="bi bi-collection"></i> 浜у搧鏁? <strong>{product_count}</strong></p>
            </div>
                    <div class="card-hint">鐐瑰嚮鏌ョ湅璇︽儏</div>
                </div>
                </div>
...
def generate_details_page(product_sales_ratio_data, daily_sales, output_dir):
    """鐢熸垚 details.html 椤甸潰
    Args:
        product_sales_ratio_data (list): 姣忔棩浜у搧浜ч攢鐜囨槑缁嗘暟鎹垪琛ㄣ€?        daily_sales (dict): 姣忔棩閿€鍞暟鎹€?        output_dir (str): HTML 鏂囦欢杈撳嚭鐩綍銆?    """
    print("寮€濮嬬敓鎴?details.html 椤甸潰...")
    page_title = "鏄ラ洩椋熷搧鐢熷搧浜ч攢鍒嗘瀽鎶ュ憡 - 璇︾粏鏁版嵁"
...
if __name__ == '__main__':
    ...
    dummy_product_ratio_details = [
        {'date': datetime(2023, 10, 25), 'data': pd.DataFrame({'鍝佸悕': ['A'], '閿€閲?: [1], '浜ч噺': [2], '浜ч攢鐜?: [50]})},
    ]
    dummy_daily_sales = {
        datetime(2023, 10, 25): {'volume': 5000, 'avg_price': 15000, 'product_count': 1, 'data': pd.DataFrame({'鐗╂枡鍚嶇О':['X']}), 'quantity_column': 'Q'},
    }
    ...
    print(f"娴嬭瘯 details.html 宸茬敓鎴愬湪 {output_directory}") 